...
const ViewEquipos = () => {
  ...
  const getEquipos = () => {
    axios.get(`/api/equipos/equiposCMM`).then((res) => {
      setEquipos(res.data);
    });
    axios.get(`/api/getconfig`).then((res) => {
      setEquiposConfig(res.data.equiposCMMData);
    });
  };

  const postEquipo = () => {
    axios
      .post(`/api/equipos/equiposCMM`, newEquipoData)
      .then((res) => {
        setNewEquipoData({});
        setNewEquipo(false);
        getEquipos();
        ...
        >
          ...
          >
            <Text fontSize={"1.5rem"} fontWeight={"600"} color={"#fff"}>
              Cabinas de Mediciones y Maniobras
            </Text>
            <Flex
              my={"30px"}
              w={"80%"}
